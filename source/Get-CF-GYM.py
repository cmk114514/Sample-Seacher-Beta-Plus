...
data = {}
try:
    with open(os.path.join('..', 'data', 'CF-GYM.json.7z'), 'rb') as f:
        data = json.loads(lzma.decompress(f.read()).decode('utf-8'))
except:
    print('WARNING: OVERLOAD')
    pass

data['URL'] = 'codeforces.com/gym/problem/{0}/{1}'
data['PID'] = 'CF {0} {1}'
...
def get(cid, pid):
    ...
    try:
        url = f'https://codeforces.com/gym/{cid}/problem/{pid}'
...
def get_and_save(cid):
    print(f'Getting {cid}...')
    try:
        url = f'https://codeforces.com/gym/{cid}'
        res = req.get(url, headers = {
            'User-Agent': 'Mozilla/4.0 (compatible; MSIE 7.0; Windows NT 10.0; WOW64; Trident/7.0; .NET4.0C; .NET4.0E; Tablet PC 2.0; wbx 1.0.0; wbxapp 1.0.0; Zoom 3.6.0)'
            })
        s = bs(res.text, 'lxml')
...
with open(os.path.join('..', 'data', 'CF-GYM.json.7z'), 'wb') as f:
